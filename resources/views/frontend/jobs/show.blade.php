...
@section('title', $job->title . ' - ' . \App\Models\Setting::get('site_name'))
    ...
    <div class="row">
        <div class="col-lg-8 mx-auto" data-aos="fade-up">
            <div class="card border-0 shadow-sm mt-5">
                @if($job->image)
                <img src="{{ asset('storage/' . $job->image) }}" class="card-img-top job-detail-img" alt="{{ $job->title }}">
                @endif
                <div class="card-body">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="background: #1a1f2e; border: 1px solid rgba(108, 92, 231, 0.3); border-radius: 12px;">
            <div class="modal-header" style="border-bottom: 1px solid rgba(108, 92, 231, 0.2);">
                <h5 class="modal-title fw-bold text-white" id="applyModalLabel">Apply for {{ $job->title }}</h5>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function formSubmitHandler(e) {
        ...
        try {
            ...
            const response = await fetch("{{ route('jobs.apply', $job->slug) }}", {
                method: 'POST',
                body: formData,
                headers: {
